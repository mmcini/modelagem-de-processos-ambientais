---
title: Exercício - Estatística Descritiva
...
A função `pnorm` retorna a probabilidade acumulada de um população. Pra usar
tem que informar qual a média e qual o desvio padrão da população:
...
Essa expressão dá os valores do eixo x para uma distribuição de média zero e desvio
padrão igual a 1. Dá pra visualizar assim:

```{r}
plot(density(dados_normalizados))
```

## Calcule os intervalos de confiança da média
...
Considerando um intervalo de confiança de 95%, como ele explicou na aula,
o z de 95% em uma distribuição normal é $\pm 1,96$. Então é só fazer:
...
As amostras que tenham entre esses valores de média tem 95% de
probabilidade de pertencerem à essa população. Dá pra visualizar:

```{r}
plot(density(dados_normalizados))  + abline(v = ic) + abline(v = -ic)
```

## Teste se essa distribuição é normal. Discuta.

Pra testar a normalidade é possível usar o teste de Shapiro-Wilk. A hipótese
...
O p-valor é menor que 0,05, então a distribuição não é normal (dá pra ver
nos gráficos acima que não é, mesmo sem o teste)

# Exercício 2
## Utilizando o dataset mtcars:

mtcars é um dataset que vem junto com o R. Dá para usar para fazer
